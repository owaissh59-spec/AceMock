...
const ActiveTest = () => {
  ...
  // Auto-fit: on desktop, scale content to fit viewport. On mobile, allow scroll.
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (naturalHeight > availableHeight && naturalHeight > 0) {
        ...
        setScaleFactor(Math.max(0.6, Math.min(1, ratio)));
      } else {
        setScaleFactor(1);
      }
    }, 50);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const availableHeight = area.clientHeight;
      const naturalHeight = content.scrollHeight / scaleFactor; // undo current scale to get natural height
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden relative">
      ...
      <div className={cn("flex-1 flex flex-col transition-all duration-300", currentSession.isPaused && "blur-md pointer-events-none")}>
        ...
        {/* Question Area */}
        <div className="flex-1 overflow-y-auto md:overflow-hidden p-4 md:p-6" ref={questionAreaRef}>
          <div
            ref={contentRef}
          ...
          >
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden mb-4">
              ...
              <div className="p-4 md:p-6 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
              ...
              <div className="p-4 md:p-6">
                <div className="prose prose-sm md:prose-base prose-slate dark:prose-invert prose-p:leading-relaxed prose-p:mb-2 prose-p:mt-0 prose-pre:bg-slate-800 prose-pre:text-slate-100 prose-th:bg-slate-100 dark:prose-th:bg-slate-800 prose-td:border-slate-200 dark:prose-td:border-slate-700 max-w-none mb-6 overflow-x-auto whitespace-pre-wrap text-slate-800 dark:text-slate-200">
